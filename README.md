...
배포 작업은 GitHub의 `production` environment에서 실행됩니다. 교체 전에 실행 중인
두 이미지의 정확한 참조를 보관하고, 컨테이너 상태·healthcheck와 `/api/health`가
모두 통과해야 성공으로 처리합니다. 교체 후 실패하면 제한된 컨테이너 로그를 남기고
두 이전 이미지가 모두 있을 때만 자동 롤백합니다. 검증 성공 후에는 현재·롤백 이미지를
보존하면서 사용하지 않는 OnMyWay 라벨 이미지(태그가 없는 교체 이미지 포함)만
이미지 ID로 정리합니다.
